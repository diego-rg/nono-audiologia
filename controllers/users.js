...
module.exports.newUser = async (req, res) => {
  try {
    ...
    //facemos un error handler extra para avisar se o nome de usuario ou email xa está en uso
    if (e.message === "A user with the given username is already registered") {
      ...
      e.message.includes(
        "E11000 duplicate key error collection: nono.users index: email_1 dup key:"
      )
    ) {
      req.flash(
        "error",
...
module.exports.loginUser = (req, res) => {
  //Middleware de passport. Xa comproba él a authentication
  req.flash("success", "Bienvenido a NoNo!");
