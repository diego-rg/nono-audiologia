...
module.exports.category = async (req, res) => {
  const sounds = await Sound.find({ category: req.params.category }).sort({
    name: "asc",
  }); //Añadimos sort para orden alfabético
  ...
  if (!categs.includes(req.params.category)) {
    //comparamos os params do cliente (se escribe na url) coas categorías. Se o array de categorías non a ten da erro
...
module.exports.showSound = async (req, res, next) => {
  ...
  if (!sound) {
    //Solo funciona cando a id foi eliminada ou cando  podería ser válida?
...
module.exports.editForm = async (req, res) => {
  //Primeiro pasar isLoggedIn para comprobar e ter acceso a user
...
module.exports.searchSound = async (req, res, next) => {
  const sound = await Sound.findOne({ name: req.query.name }).collation({
    locale: "es",
    strength: 1,
  }); //collation strength 1 para que non distinga minúsculas de maiúsculas
  if (!sound) {
    req.flash("error", "El sonido que indica no existe.");
    return res.redirect("/sounds");
  }
